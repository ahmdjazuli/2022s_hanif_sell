          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h2 style="display:inline;">Barang Rusak</h2>
                <button style="float: right;margin-left: 5px" class="btn btn-dark" type="button" data-toggle="tooltip" data-placement="bottom" title="Tambah"><a href="tanamrusak_input.php" class="text-white"><i class="fas fa-folder-plus"></i></a>
                </button>
              ...
              <div class="card-body">
                <table id="dataTables" class="table table-bordered table-hover table-sm">
                  ...
                  <tbody>
                    <?php 
                      $no = 1;
                      $query = mysqli_query($kon, "SELECT * FROM tanamrusak INNER JOIN tanam ON tanamrusak.idtanam = tanam.idtanam ORDER BY tgl DESC");
                      while($data = mysqli_fetch_array($query)){
                        ?>
                          ...
                          <td><?= $data['catatan'] ?></td>
                          <td>
                            <button class="btn bg-dark" type="button"><a href="tanamrusak_edit.php?idtanamrusak=<?= $data['idtanamrusak'] ?>" class="text-white"><i class="far fa-edit"></i></a></button>
                            <button class="btn bg-dark" onclick="yakin = confirm('Apakah Kamu yakin ingin Menghapus?'); if(yakin){ window.location = 'hapus.php?idtanamrusak=<?= $data['idtanamrusak'] ?>';
